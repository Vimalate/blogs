...
3. 开发排期
4. 编码开发
5. 编码过程当中，测试会组织一次测试用例评审。
6. 冒烟测试（自检验）
7. 冒烟通过，提交测试，合并代码到测试分支，部署测试环境
8. 测试环境测试，开发修 bug
9. 测试完成，提交预发，合并代码到预发分支，部署预发环境
10. 预发环境测试，开发修 bug（修完的 bug 要重新走测试再走预发，这个下面会解释）
11. 测试完成，产品验收
12. 验收完成，提交生产，合并代码到生产分支，部署生产环境
13. 生产运营（客户）验收
14. 验收完成，结项
